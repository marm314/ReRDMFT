...
// DEBUG-only: validates Hessian_opt/GeneralizedHessian.h's analytic
// Hess_pq,rs against a genuine 2D finite difference
...
std::string hessianFiniteDifferenceReport(const std::string& label, const rerdmft::Matrix<T>& h,
  ...
  out << "\n"
      ...
      << ",r=" << r_idx << ",s=" << s_idx << "):\n";
  out << "  Analytic Hess_pq,rs (Hessian_opt/GeneralizedHessian.h): "
  ...
  try {
    ...
    if (input.non_relativistic()) {
      ...
      if (input.debug()) {
        ...
        // (Hessian_opt/GeneralizedHessian.h) -- only when the O(n^5)
        // dense-2-RDM path above actually ran (needs fock_spin/d_spin/
        // two_rdm_spin) and a THIRD distinct occupied index is
        // available, so (p,q)=(lumo,homo_minus_1) and (r,s)=(lumo,0)
        // are genuinely different pairs sharing the same virtual
        // (lumo). Deliberately spatial-orbital index 0, not
        // n_occ_spatial-3: for water/STO-3G, pairing lumo (4a1) with
        // n_occ_spatial-3 (=2, 1b2) is ALSO symmetry-zero (same
        // phenomenon as HOMO/LUMO's gradient above -- confirmed
        // empirically: analytic and numerical both read ~0 there,
        // uninformative), while index 0 IS symmetry-allowed and gives a
        // genuinely nonzero element that tracks convergence, confirmed
        // against water at 1/2 iterations and fully converged.
        if (nonrel_gradient_computed && n_spatial > static_cast<std::size_t>(n_occ_spatial) &&
            n_occ_spatial > 2) {
          const std::size_t homo_minus_1 = static_cast<std::size_t>(n_occ_spatial) - 2;
          const std::size_t second_occ = 0;
          const std::size_t lumo = static_cast<std::size_t>(n_occ_spatial);
          const double analytic_hess = rerdmft::generalizedOrbitalHessianElement(
          ...
          nonrel_hessian_report = hessianFiniteDifferenceReport(
              ...
              nonrel_hf_result.nuclear_repulsion_energy, analytic_hess);
        }
      }
    }
