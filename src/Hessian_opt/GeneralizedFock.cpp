...
Matrix<T> generalizedFockMatrix(const Matrix<T>& h, const Tensor4<T>& eri, const Matrix<T>& d,
  ...
  //
  // F_pq = sum_r h_rp D_rq + sum_rst (st|rp) P_strq  (Dyall & Faegri
  // Eq. 8.30's first sum -- see GeneralizedFock.h): eri(s,t,r,p) is the
  // chemist-notation integral (st|rp), two_rdm(s,t,r,q) is Dyall's
  // P_strq (Eq. 8.28's index convention).
      ...
      for (std::size_t r = 0; r < n; ++r) {
        for (std::size_t s = 0; s < n; ++s) {
          for (std::size_t t = 0; t < n; ++t) {
            two_electron += eri(s, t, r, p) * two_rdm(s, t, r, q);
          }
        }
      }
