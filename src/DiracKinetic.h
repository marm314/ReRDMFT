...
// every nucleus in `geometry`. Since it is a scalar potential (identity
// across all 4 spinor components), it is block-diagonal with the same
// <Large_i|Vext|Large_j> matrix repeated for both Large spin blocks and
// the same <Small_i|Vext|Small_j> matrix repeated for both Small spin
// blocks, with zero Large-Small coupling.
