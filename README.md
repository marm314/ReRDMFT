...
| `MIXING` | double, in `(0, 1]` | `0.4` | Linear density-matrix mixing weight of the Hartree-Fock SCF loops, used only with `DIIS FALSE` (DIIS is the default): the density fed into the next iteration is `mixing*P_new + (1-mixing)*P_current`. |
| `DIIS` | bool | `TRUE` | Use Pulay's DIIS (`Utils/DIIS.h`) instead of the linear density mixing in the Hartree-Fock SCF loops of all three paths (`NON_RELATIVISTIC`, `X2C`, `C4_SPINOR`; real orbitals for `NON_REL`, complex for the others). The error vector is the commutator of the Fock matrix with the density in the AO basis, `F P S - S P F` (`S` = `S_Large`, its Large-component spin-orbital block for `X2C`, `S_full` for `C4_SPINOR`); the Fock matrix built from the density of the previous iteration is extrapolated from the last `DIIS_SIZE` (error, Fock) pairs -- extrapolation starts at the second iteration -- and the extrapolated matrix is diagonalized, with no density mixing on top (`MIXING` is then ignored). The weights are real (real part of the Hermitian inner product of the errors), which keeps the extrapolated Fock matrix Hermitian and time-reversal symmetric, unlike a complex-weight variant. Typically 5-10x fewer iterations (water: 38 -> 7) and a tighter converged energy/gradient than the linear mixing at the same tolerances. Like any DIIS it converges to *a* stationary point of the SCF equations, which is not always the one the damped linear mixing finds: for stretched LiH (5 Angstrom, `examples/lih_X2C_gnof_full_optimization_dissociated.inp`) the two settings reach different solutions (X2C-HF total energies -7.8241 vs -7.7990 Ha), that example therefore sets `DIIS FALSE`, which selects the linear density mixing (`MIXING`), which is the more robust choice for such near-degenerate, stretched cases. |
...
| `ORBITAL_GRADIENT_TOLERANCE` | float | `1e-5` | ADAM's (or NEO's) orbital-gradient convergence threshold (max gradient entry; `10**-itolLambda` of the Fortran code). |
| `ORBITAL_OPTIMIZER` | string | `ADAM` | Which method drives `FULL_OPTIMIZATION`'s orbital-rotation step. `ADAM`: DoNOF's own optimizer, described above. `NEO`: `Utils/NEO.h`'s matrix-free, second-order trust-region Newton method, targeting the ground state only (no saddle-point search); the orbital-rotation Hessian is never formed densely -- a row-based Hessian-vector product (`Hessian_opt/JkOnlyHessian.h`'s/`HartreeExchangeHessian.h`'s `...JointHessianVector`, or a symmetrized per-element sum for real orbitals) costs `O(n_pairs)` memory instead of `O(n_pairs^2)`. Each macro-iteration's Newton descent runs to ITS OWN full convergence (an earlier ADAM-style small-then-growing budget was tried and measured to only ever hurt: cutting a descent short can freeze the whole macro loop into a worse answer, e.g. 2e-5 Ha off on `lih_pnof7`'s X2C branch; removed). Works with `CHOLESKY TRUE` too (the Hessian-vector product is generic over the two-electron integral representation, `Tensor4` or `Utils/CholeskyEri.h`'s vectors -- verified to match the dense-integral result to machine precision on both NON_REL and X2C); falls back to ADAM (with a printed note) only if the model somehow has no Hessian-vector callback, which should not occur in practice. `CHOLESKY TRUE` + NEO materializes ONE dense tensor per ACCEPTED Newton step (`CholeskyEri::toDense()`) and reuses it for every Hessian-vector product that step's Davidson solve needs, rather than paying `CholeskyEri::operator()`'s `O(n_chol)`-per-element cost on each of those calls directly -- without this caching a single macro-iteration did not finish in 5 minutes on a small (LiH/6-31G) system; with it the whole run takes about as long as the dense-integral case. Typically converges in far fewer macro-iterations than ADAM (a handful, not tens) and matches ADAM's converged energy to 1e-9-1e-6 on JK_only functionals and on X2C in every case tried.  **PNOF/GNOF, mainly on NON_REL:** NEO's Newton trajectory can converge to a genuinely DIFFERENT stationary point than ADAM's, by 1e-5 to 1e-3 Ha depending on the system -- confirmed, not a numerical error nor an iteration-budget artifact (checked directly: neither a wider budget nor forcing full, untruncated convergence closes the gap). After the loop, `Utils/NEO.h`'s `neoLowestHessianEigenpairs` checks whether the point NEO stopped at is a genuine minimum of the orbital-rotation Hessian (0 negative eigenvalues) within the same restricted subspace it searched (Kramers for X2C, spin-restricted for NON_REL): on the systems checked, the LARGER discrepancies (`water_gnof`, `co_sto3g_gnof`, ~3e-3 Ha) are confirmed genuine alternate MINIMA (all eigenvalues ~0, machine precision), while the SMALLER ones (`lih_pnof5`/`lih_pnof7`/`lih_pnof7s` NON_REL, ~2e-5 Ha) are confirmed shallow SADDLES (one eigenvalue around -2e-5, matching the energy gap almost exactly) -- a flat direction NEO's gradient tolerance is satisfied on before it is fully descended. Compare against an `ORBITAL_OPTIMIZER ADAM` run as a routine cross-check for PNOF/GNOF; see `examples/water_muller_as_neo_full_optimization.inp`, `examples/lih_gnof_neo_full_optimization.inp` and `examples/water_X2C_neo_full_optimization.inp` for template inputs (the ADAM/NEO pairs of each also make a direct comparison easy). |
